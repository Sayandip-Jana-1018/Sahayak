...
import {
  LayoutDashboard, Users, Building2, Brain, AlertTriangle, Server, Megaphone, ChevronLeft, Bell,
} from 'lucide-react';
...
const ADMIN_NAV = [
  { href: '/admin', icon: LayoutDashboard, label: 'Overview', color: '#6366F1' },
  { href: '/admin/users', icon: Users, label: 'Users', color: '#10B981' },
  { href: '/admin/organizations', icon: Building2, label: 'Orgs', color: '#F59E0B' },
  { href: '/admin/ai-usage', icon: Brain, label: 'AI Usage', color: '#8B5CF6' },
  { href: '/admin/sos', icon: AlertTriangle, label: 'SOS', color: '#EF4444' },
  { href: '/admin/system', icon: Server, label: 'System', color: '#06B6D4' },
  { href: '/admin/announcements', icon: Megaphone, label: 'Announce', color: '#F97316' },
];
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="dash-shell">
      ...
      <nav className="side-nav" aria-label="Admin navigation" role="navigation">
        <Link href="/dashboard" className="side-nav__back" title="Back to Dashboard">
          <ChevronLeft size={16} />
        </Link>
        <div className="side-nav__logo side-nav__logo--admin">A</div>
        ...
        {ADMIN_NAV.map(({ href, icon: Icon, label, color }) => {
          ...
          return (
            <Link key={href} href={href} className="side-nav__link" data-active={active || undefined}>
              <span className="side-nav__icon-bubble" style={{
                color, background: active ? hexToRgba(color, 0.15) : 'transparent',
              ...
              {active && <span className="side-nav__bar" style={{ background: color, boxShadow: `0 0 12px ${color}` }} />}
            </Link>
          );
        })}
      </nav>

      <nav className="bot-nav" aria-label="Admin navigation">
        <div className="bot-nav__track">
          {ADMIN_NAV.slice(0, 5).map(({ href, icon: Icon, label, color }) => {
            const active = isActive(href);
            return (
              <Link key={href} href={href} className="bot-nav__tab" data-active={active || undefined}>
                <span style={{ color, display: 'flex' }}><Icon size={20} strokeWidth={active ? 2.4 : 1.8} /></span>
